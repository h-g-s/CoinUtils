...
void
cliqueDetection(CGraph *cgraph, const std::vector<std::pair<int, double> > &columns, double sumNegCoefs, double rhs,
...
void mixedCliqueDetection(CGraph *cgraph, const std::vector<std::pair<int, double> > &columns, double sumNegCoefs,
    ...
    for (int i = 0; i < nElements - 1; i++) {
        ...
        double coef = columns[i].second;
        double maxLHS =
                sumNegCoefs - std::min(0.0, coef) - std::min(0.0, columns[nElements - 1].second) +
