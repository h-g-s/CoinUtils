  ...
  if (clqem == CLQEM_PRIORITY_GREEDY) { //clique extender method uses greedy selection (reduced cost)
    ...
    for (int i = 0; i < nNeighs; i++) {
      ...
      for (int j = 0; j < clqe->newCliqueSize; j++) {
        int complement = (clqe->newClique[j] < nCols) ? (clqe->newClique[j] + nCols) : (clqe->newClique[j] - nCols);
        if ((!cgraph_conflicting_nodes(cgraph, clqe->newClique[j], selected)) || (selected == clqe->newClique[j]) || (selected == complement)) {
          insert = false;
          break;
        }
      }
...
int exact_clique_extension(CliqueExtender *clqe, const int *clique, const int size, const int weight)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < size; j++) {
      ...
      if ((neigh == clique[j]) || (neigh == complement) || (!cgraph_conflicting_nodes(clqe->cgraph, clique[j], neigh)))
        break;
    }
    if (j >= size) {
